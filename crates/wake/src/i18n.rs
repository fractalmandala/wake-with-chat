...
mod tests {
    ...
    /// Settings → Agent access 把 ACP 方言表的 CLI 登录指引过一道 `t()`
    /// (`t(dialect.auth_hint)`),英文原文在 wake-core::services::acp 的方言
    /// 表里。与 setup_snippets 同款棘轮:这里直接问那个表,改了措辞就在
    /// 这里红,而不是静默死译文
    fn from_acp_dialects(key: &str) -> bool {
        wake_core::models::AgentId::ALL
            .iter()
            .copied()
            .filter_map(wake_core::services::acp::acp_dialect)
            .any(|d| d.auth_hint == Some(key))
    }
