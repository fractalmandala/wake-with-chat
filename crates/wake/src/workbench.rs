...
use wake_core::services::{exporter, handoff, terminal};
...
pub struct Workbench {
    ...
    /// In-app chat 登记簿缓存(侧栏 Chats 区):面板写盘后经 RegistryDirty 重读
    chat_records: Vec<crate::chat_registry::ChatRecord>,
    /// session_id → agent 库里模型起好的标题;登记簿的首句标题只是回退
    chat_titles: HashMap<String, String>,
    chats_collapsed: bool,
    /// All Sessions 页头的分段切换:true 时中栏改列全部聊天(Chats 页签)
    chats_tab: bool,
    starred_count: i64,
    ...
    insights_open: bool,
    /// In-app chat(ACP):右侧第四栏。None = 未开过;丢弃 entity 即收掉
    /// agent 子进程(Drop → shutdown)。与会话选择无状态耦合:cwd 在打开
    /// 那一刻取详情页的项目目录
    chat_panel: Option<Entity<crate::chat::ChatPanel>>,
...
impl Workbench {
    pub fn new(window: &mut Window, cx: &mut Context<Self>) -> Self {
        ...
        let mut this = Self {
            ...
            // 登记簿是几十条小 JSON,启动同步读一次,之后靠 RegistryDirty 增量重读
            chat_records: crate::chat_registry::load(),
            chat_titles: HashMap::new(),
    ...
    fn refresh(&mut self, cx: &mut Context<Self>) {
        ...
        // 会话索引刚更新:chat 登记簿里那些首句标题,agent 库侧可能已由
        // 模型起好名,顺带对齐
    ...
    /// 左栏 Chats 显示数据:重读登记簿,并用 agent 库里模型起的标题覆盖
    /// 首句标题("hi" 这类)。登记簿只存指针,agent 自己的库是唯一事实
    /// 源,标题同源——聊天代理自己就是 LLM,它写库时的命名比截断首句强
    fn refresh_chat_records(&mut self) {
        self.chat_records = crate::chat_registry::load();
        let mut by_agent: HashMap<&str, Vec<String>> = HashMap::new();
        ...
        for (agent, ids) in by_agent {
            if let Ok(map) = self.store.titles_by_native_ids(agent, &ids) {
                // 库里的占位名(未起名前的 "New session - <时间戳>"、"Untitled")
                // 不覆盖——首句截断虽弱也比占位符有信息量;agent 真正命名后
                // 下次索引同步就会带进来
                titles.extend(
                    map.into_iter()
                        .filter(|(_, title)| !is_placeholder_title(title)),
                );
            }
        }
    ...
    /// In-app chat 开关(⌘⇧C / 侧栏底部按钮)。无可用 ACP agent 时只提醒
    /// 不开面板——空面板是死按钮,说一声为什么(链接 Settings 在 M4 polish)
    fn toggle_chat(&mut self, window: &mut Window, cx: &mut Context<Self>) {
        if self.chat_panel.take().is_some() {
            cx.notify();
            return;
        }
        let targets = wake_core::services::acp::acp_targets();
        let Some(agent) = targets.first().copied() else {
            window.push_notification(
                Notification::warning(t(
                    "No chat-capable agent found — install one with ACP support (kimi, opencode, cursor-agent, gemini)",
                )),
                cx,
            );
            return;
        };
        // cwd:详情页会话的项目目录(存在且可用),否则 HOME——agent 在哪
        // 个工作区干活由打开那一刻的选中会话决定
        let cwd = self
            .detail
            .as_ref()
            .map(|d| PathBuf::from(&d.meta.project_path))
            .filter(|p| p.is_dir())
            .or_else(dirs::home_dir)
            .unwrap_or_else(|| PathBuf::from("."));
        self.open_chat_panel(agent, cwd, None, window, cx);
    }

    /// 打开(或换会话重开)聊天面板。换 agent/cwd/resume 都是换会话:
    /// 旧面板直接丢弃(旧 agent 子进程随之收掉),事件订阅跟随新面板重建
    fn open_chat_panel(
        &mut self,
        agent: AgentId,
        cwd: PathBuf,
    ...
    /// 侧栏 Chats 点进来的续聊:登记时声明支持回放才带 resume,
    /// 否则用同 agent/cwd 开新对话(点击不该是无反应)
    fn open_saved_chat(
        &mut self,
        record: crate::chat_registry::ChatRecord,
        window: &mut Window,
        cx: &mut Context<Self>,
    ) {
        let Some(agent) = AgentId::from_str(&record.agent) else {
            return;
        };
        ...
        let resume = record.can_resume.then(|| record.session_id.clone());
        self.open_chat_panel(agent, cwd, resume, window, cx);
    }
    ...
    fn set_scope(
        ...
        self.insights_open = false;
        // 页签只在无过滤的 All Sessions 视图有意义,任何过滤切换都回 Sessions
        self.chats_tab = false;
        self.refresh(cx);
    }
    ...
    /// 跨 agent 接力(Continue with…):导出交接稿 → 目标 agent CLI 读稿
    /// 续作。终端宿主沿用 Open In 的记忆(同一根分栏按钮,目标列表不同);
    /// 后台执行写稿 + 起终端,完成桥与 do_resume 同款
    fn do_handoff(
        &mut self,
        target: handoff::HandoffTarget,
        window: &mut Window,
        cx: &mut Context<Self>,
    ) {
        let Some(detail) = &self.detail else { return };
        ...
        let Some(term) = self.open_in_target(meta.agent, &terms) else {
            window.push_notification(
                Notification::warning(t("No terminal application found on PATH")),
                cx,
            );
            return;
        };
        let task = cx.background_spawn(async move {
            adapter_for(&adapters, meta.agent, &meta.file_path)
                .ok_or_else(|| "no adapter for this session".to_string())
                .and_then(|adapter| handoff::continue_with(adapter, &meta, target, term))
        });
        Self::notify_when_done(window, cx, task, move |result| match result {
            Ok(message) => Notification::success(message),
            Err(error) => Notification::error(error),
        });
    }

    /// Continue in Wake:导出交接稿 → 面板内开(或复用)目标 agent 的 ACP
    ...
    fn do_continue_in_wake(
        ...
        let meta = detail.meta.clone();
        let adapters = self.adapters.clone();
        // 复用同 agent 面板;否则新开(丢弃旧面板即收掉旧 agent 子进程)
        match &self.chat_panel {
            Some(p) if p.read(cx).agent() == agent => {}
            _ => {
                let cwd = PathBuf::from(&meta.project_path);
                let cwd = if cwd.is_dir() {
                    cwd
                } else {
                    dirs::home_dir().unwrap_or_else(|| PathBuf::from("."))
                };
                self.chat_panel = Some(crate::chat::ChatPanel::open(agent, cwd, None, window, cx));
            }
        };
        let task = cx.background_spawn(async move {
            adapter_for(&adapters, meta.agent, &meta.file_path)
                .ok_or_else(|| anyhow::anyhow!("no adapter for this session"))
                .and_then(|adapter| handoff::write_handoff(adapter, &meta))
        });
        cx.spawn_in(window, async move |this, cx| {
            let result = task.await;
            let _ = this.update_in(cx, |this, window, cx| match result {
                Ok(path) => {
                    let prompt = handoff::handoff_prompt(&path);
                    this.chat_panel
                        .as_ref()
                        .map(|panel| panel.update(cx, |p, cx| p.prefill(&prompt, window, cx)));
                }
                Err(e) => {
                    window.push_notification(
                        Notification::error(crate::tf!("Handoff failed: {:#}", e)),
                        cx,
                    );
                }
            });
        ...
        v_flex()
            ...
            .child(
                v_flex()
                    ...
                    // In-app chat 登记簿:每条可从 session/load 续聊(面板负责
                    // 写盘,这里只读缓存)。空态一行灰字,不整段隐藏入口
                    .child(group_header(
                        "chats-header",
                        t("Chats"),
                        self.chats_collapsed,
                        cx.listener(|this, _, _window, cx| {
                            this.chats_collapsed = !this.chats_collapsed;
                            cx.notify();
                        }),
                        cx,
                    ))
                    .when(!self.chats_collapsed, |this| {
                        this.when(self.chat_records.is_empty(), |this| {
                            this.child(
                                div()
                                    .px(SPACE_XL)
                                    .py(SPACE_XS)
    ...
    /// All Sessions 页头的 Sessions/Chats 分段切换。轨道 muted、选中片
    /// popover 底 + 常规阴影浮起;内圆角 = 外圆角 − 轨道 padding(同心)
    fn render_scope_tabs(&self, cx: &Context<Self>) -> AnyElement {
        let theme = cx.theme();
        let inner_radius = theme.radius - px(2.);
        let tab = |id: &'static str, label: SharedString, active: bool, to_chats: bool| {
            div()
                .id(id)
                .px(SPACE_SM)
                .py(px(3.))
                ...
                .on_click(cx.listener(move |this, _, _window, cx| {
                    if this.chats_tab != to_chats {
                        this.chats_tab = to_chats;
                        // 聊天列表可能落后于刚才的会话(标题/模型徽章),切进来现读
                        if to_chats {
                            this.refresh_chat_records();
                        }
                        cx.notify();
    ...
    /// Chats 页签正文:登记簿全量(已按 updated_at 降序),行式布局与会话行
    /// 同构——标题一行,副行 agent 图标 + 模型名 + 相对时间。点击续聊
    fn render_chats_tab(&self, cx: &Context<Self>) -> AnyElement {
        let theme = cx.theme();
        if self.chat_records.is_empty() {
            return v_flex()
                .flex_1()
                .w_full()
                .items_center()
        ...
        v_flex()
            ...
            .children(self.chat_records.iter().enumerate().map(|(ix, r)| {
                let record = r.clone();
                // 标题三层回退与侧栏 Chats 组一致:agent 库里 LLM 起的名 →
                // 登记簿首句截断 → 未命名
                let title = self
                    .chat_titles
                    .get(&record.session_id)
                ...
                // 模型名只留最后一段(provider/model 的 model 部分);没登记过
                // 模型的旧条目退回 agent 显示名,行副行永远有个来源标签
                let source = record
                    .model
                    .as_deref()
                    ...
                    .or_else(|| {
                        Some(
                            AgentId::from_str(&record.agent)
                                .map(|a| a.display_name().to_string())
                                .unwrap_or_else(|| record.agent.clone()),
                        )
                    });
                let time: SharedString = smart_time(record.updated_at).into();
                let time_tooltip: SharedString = abs_date(record.updated_at).into();
    ...
    fn render_session_list(&self, cx: &Context<Self>) -> impl IntoElement {
        ...
        let heading: SharedString = if chats_tab {
            ...
            self.context_title().into()
        };
        let shown_label: SharedString = if chats_tab {
            let n = self.chat_records.len();
            format!("{} {}", n, if n == 1 { "chat" } else { "chats" }).into()
        } else {
            format!(
                "{} {}",
                listed_count,
                            ...
                            .child(
                                h_flex()
                                        ...
                                        terminal::resume_targets(meta).as_slice()
                                    {
                                        ...
                                        // 接力目标在菜单展开时现查(有 PATH 探测缓存,不付代价):
                                        // 刚装好的 CLI 不用重启就能出现在列表里
                                        let src_agent = meta.agent;
                                            ...
                                            .child(
                                                Button::new("open-in-more")
                                                    ...
                                                    .dropdown_menu(move |menu, _, _| {
                                                        ...
                                                        // Continue with…:跨 agent 接力(handoff 模块),与
                                                        // Open In 的终端列表分段;点击 = 导出交接稿 + 在原项目
                                                        // 目录拉起目标 agent 读稿续作
                                                        let targets = handoff::handoff_targets(src_agent);
                                                        if !targets.is_empty() {
                                                            menu = menu.item(PopupMenuItem::separator());
                                                            menu = menu.item(PopupMenuItem::label(t("Continue with")));
                                                            for ht in targets {
                                                                let entity = menu_entity.clone();
                                                                let color =
                                                                    crate::theme::agent_series_color(ht.agent);
                                                                menu = menu.item(
                                                                    PopupMenuItem::element(move |_, _| {
                                                                        h_flex()
                                                                            .gap(SPACE_SM)
                                                                            ...
                                                                            .child(ht.agent.display_name())
                                                                    })
                                                                    .on_click(move |_, window, cx| {
                                                                        entity.update(cx, |this, cx| {
                                                                            this.do_handoff(ht, window, cx);
                                                                        });
                                                                    }),
                                                                );
                                                            }
                                                        }
                                                        // Continue in Wake:导交接稿 → 面板内开 ACP
                                                        // 会话,预填读稿提示。目标 = 面板 agent
                                                        // picker 同一张方言表可用列表
                                                        let acp = wake_core::services::acp::acp_targets();
                                                        if !acp.is_empty() {
                                                            menu = menu.item(PopupMenuItem::separator());
                                                            menu = menu.item(PopupMenuItem::label(t("Continue in Wake")));
                                                            for a in acp {
                                                                let entity = menu_entity.clone();
                                                                let color = rgb(crate::theme::agent_series_color(a));
                                                                menu = menu.item(
                                                                    PopupMenuItem::element(move |_, _| {
                                                                        h_flex()
                                                                            .gap(SPACE_SM)
                                                                    ...
                                                                    .on_click(move |_, window, cx| {
                                                                        entity.update(cx, |this, cx| {
                                                                            this.do_continue_in_wake(a, window, cx);
                                                                        });
                                                                    }),
                                                                );
                                                            }
...
/// agent 库未起名前的占位标题:opencode 的 "New session - <ISO 时间戳>"、
/// gemini 的 "Untitled"。这类名字比首句截断更没信息量,Chats 列表不采用
        ...
        .child(
            h_flex()
                ...
                .when_some(tag, |this, tag| {
                    // 模型等附加标签:透明底描边胶囊,与 count 数字胶囊区分
                    this.child(outline_badge(tag, theme.muted_foreground))
                }),
        )
}
        ...
        div()
            ...
            .child(
                h_flex()
                    ...
                    // chat 是独立第四栏:Insights 全页替换中右栏时也保持在场
                    .map(|this| match &self.chat_panel {
                        Some(panel) => this.child(panel.clone()),
                        None => this,
                    }),
            )
