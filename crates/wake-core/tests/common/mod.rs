...
pub fn stage_dir_fixtures(home: &Path) {
    for (src, dst) in [
        ...
        ("codebuddy", ".codebuddy"),
        ("codebuddy", ".workbuddy"),
        // Cowork 的沙盒会话:fixture 根即 local-agent-mode-sessions,本地
        // adapter 的家在 Library/Application Support 下,这些本地摆位只供
        // 远程同步管线(挂载点 .claude/local-agent-mode-sessions)消费
        ("cowork", ".claude/local-agent-mode-sessions"),
    ] {
