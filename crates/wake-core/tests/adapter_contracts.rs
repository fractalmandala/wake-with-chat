...
/// 数据源(CLI 的 agent-transcripts 与 IDE 的 state.vscdb),Antigravity 也是
/// 两源(CLI summaries 与桌面端 conversations 转录),Kimi 亦是(CLI 的
/// ~/.kimi-code 与 Kimi Work 桌面 home),各占一个实例。新增 agent 或给某家
/// 再加数据源时,这个数跟着加——契约要卡的是"漏了实例就爆",而不是
/// "每家恰好一个"
const DEFAULT_INSTANCES: usize = AgentId::ALL.len() + 3;
...
fn adapter_ix_for_routes_to_owning_instance() {
    ...
    assert_eq!(
        ...
        Some(2), // Cowork 插在 Claude 后,Codex 首个实例随之后移
        "无根命中回退该 agent 首个实例"
    );
}
