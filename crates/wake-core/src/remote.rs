...
pub const REMOTE_LAYOUTS: &[RemoteAgentLayout] = &[
    ...
    RemoteAgentLayout {
        ...
        // Cowork 是 Claude Desktop 的沙盒会话,数据在 macOS 的 Application
        // Support 下,没有 home 相对路径可同步;占位行只为“每家一行”的契约
        // 成立——远程机上该路径不存在,同步结果为空,无副作用
        mount: ".claude/local-agent-mode-sessions",
        sync_paths: &[".claude/local-agent-mode-sessions"],
        exclude: &[],
    },
];
