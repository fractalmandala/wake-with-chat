...
pub enum AgentId {
    ClaudeCode,
    /// Claude Desktop 的 Cowork 本地代理(local-agent-mode-sessions);
    /// 数据是 claude-format JSONL,解析复用 claude 的核心
    Cowork,
    Codex,
    Grok,
    Dsh,
...
impl AgentId {
    ...
    pub fn brand_icon(&self, dark: bool) -> &'static str {
        match self {
            ...
            // Cowork 是 Claude 家族,无独立品牌素材,共用 Claude 图标
            AgentId::Cowork => "brands/claude-code.png",
