...
// 剪贴板写接力(handoff)要复用:ClipboardOnly 形制把 prompt 放上剪贴板
pub(crate) use platform::copy_to_clipboard;
...
/// 指定 bin 名解析(ACP 侧 claude 走 npx 适配器等非常规 bin 用)
pub(crate) fn cli_bin_path(bin: &str) -> Option<String> {
    resolve_cli(bin)
}
...
pub fn agent_bin(agent: AgentId) -> Option<&'static str> {
    match agent {
        ...
        // Cowork 跑在 Claude Desktop 沙盒里,无对应 CLI
        AgentId::Cowork => None,
...
fn resume_args(meta: &SessionMeta) -> Option<(Vec<String>, bool)> {
    ...
    match meta.agent {
        ...
        AgentId::Workbuddy | AgentId::Cowork => None,
...
/// 接力(handoff)与 resume 共用的展示/启动管线。command 由同一条
/// sh_command_line 规则拼出(toast 展示与手动重跑同源),launch 由同一
/// 平台原语执行——resume_session_in 的深链分支不参与:接力只发生在
/// CLI 型 agent 之间,没有可深链的目标
pub(crate) fn compose_in(
    term: TerminalApp,
    cli: &str,
    args: &[String],
    cwd: Option<&str>,
) -> String {
    compose_command(term, cli, args, cwd)
}
