...
///
/// 另有两处同家数据源：Qoder IDE 的任务执行记录
/// `~/Library/Application Support/Qoder/SharedClientCache/cli/projects/
/// task-*.session.execution.jsonl`(parts/data 方言,parse_qoder_task_jsonl),
/// 与多工作区安装的 `~/.qoderwork/projects`(与主 projects 树同方言)。
pub struct QoderAdapter {
    /// roots[0] = 主 projects 树,其余为附加数据源;各家根独立降级(缺根 = 空)
    roots: Vec<PathBuf>,
}

/// 附加数据源的文件名后缀(task-<id>.session.execution.jsonl)
const TASK_SUFFIX: &str = ".session.execution.jsonl";
...
impl QoderAdapter {
    pub fn new() -> Self {
        ...
        Self {
            roots: vec![
                root,
                // IDE 任务执行记录与多工作区安装;缺根由扫描降级为空
                home.join("Library/Application Support/Qoder/SharedClientCache/cli/projects"),
                home.join(".qoderwork/projects"),
            ],
        }
    }

    /// 主(projects 树方言)根;任务根在尾部
    fn task_roots(&self) -> impl Iterator<Item = &PathBuf> {
        self.roots.iter().skip(1)
    }

    fn is_task_file(name: &str) -> bool {
        name.ends_with(TASK_SUFFIX)
    }
}
...
/// `<task-…>.session.execution-session.json` 边车:标题与 working_dir 的权威
/// 来源(执行记录行内不带 cwd;任务目录名是编码路径,边车缺失时才退回解码)。
struct TaskSidecar {
    title: String,
    working_dir: String,
    created_at: i64,
...
fn read_task_sidecar(jsonl: &Path) -> TaskSidecar {
    let mut out = TaskSidecar {
        title: String::new(),
    ...
    let sidecar = jsonl.with_file_name(format!("{}-session.json", stem));
    ...
    out.title = optional_string(v.get("title")).unwrap_or_default();
    out.working_dir = optional_string(v.get("working_dir")).unwrap_or_default();
    out.created_at = v.get("created_at").and_then(Value::as_i64).unwrap_or(0);
    out.updated_at = v.get("updated_at").and_then(Value::as_i64).unwrap_or(0);
    out
}

/// 编码项目目录名(`-Users-someone-Project`)退回解码;编码不转义 '-',
/// 原路径带连字符时会失真,但只是边车缺失时的退路。
fn decode_task_project_dir(jsonl: &Path) -> String {
    let dir = match jsonl.parent().and_then(|p| p.file_name()) {
...
/// 工具结果内容是字符串,但常是再包一层的 JSON(如
/// `[{"content":"$ …"}]`);能解就抽 content 拼接,解不开按原文用。
fn task_tool_output(raw: &str) -> String {
    let Ok(v) = serde_json::from_str::<Value>(raw) else {
...
/// 任务执行记录方言(parse_qoder_jsonl 的姊妹):没有 uuid/parentUuid 消息
/// 树,行按文件顺序即时间序;`role` 为 user/assistant/tool,内容在 `parts`
/// 数组(text/reasoning/tool_call/tool_result/finish/image_url)。tool_call
/// 的 input 是 JSON 字符串,tool_result 行回填到对应 tool_call;无用量数据,
/// 标题/cwd 取自 -session.json 边车。
fn parse_qoder_task_jsonl(path: &Path, decode_images: bool) -> Result<QoderParse> {
    let _image_budget = transcript_image_decode_budget(decode_images);
    let file = fs::File::open(path)?;
    let reader = BufReader::with_capacity(1 << 20, file);
    ...
    for line in reader.lines() {
        ...
        let ts = row.get("created_at").and_then(Value::as_i64).unwrap_or(0);
        if ts > 0 {
            if created_at == 0 {
                created_at = ts;
            }
        ...
        if let Some(value) = optional_string(row.get("model")) {
            model = Some(value);
        }
        let parts = row.get("parts").and_then(Value::as_array);
        ...
        if role == "assistant" {
            ...
            for part in parts {
                ...
                match part.get("type").and_then(Value::as_str) {
                    Some("text") => {
                        if let Some(text) = optional_string(data.get("text")) {
                            content.push_text(&text);
                        }
                    }
                    Some("reasoning") => {
                        if let Some(text) = optional_string(data.get("thinking")) {
                            thinking.push(text);
                        }
                    }
                    Some("tool_call") => {
                    ...
                    // finish/image_url 等对 assistant 行无展示价值
                    _ => {}
                }
            }
            ...
            let (text, truncated) = clip(content.text.trim(), MAX_MSG_TEXT);
            let thinking = if thinking.is_empty() {
                None
            } else {
                Some(clip(&thinking.join("\n\n"), MAX_TOOL_IO).0)
            };
            let msg_ix = messages.len();
            register_tools(msg_ix, &tool_calls, &mut tool_index);
            messages.push(TranscriptMessage {
                seq: 0,
                role: Role::Assistant,
                kind: MessageKind::Text,
        ...
        if role == "user" || role == "tool" {
            if role == "tool" {
                ...
                for part in parts {
                    if part.get("type").and_then(Value::as_str) == Some("tool_result") {
                        let data = part.get("data").unwrap_or(&Value::Null);
                        if let Some(id) = optional_string(data.get("tool_use_id")) {
                            let raw = optional_string(data.get("content")).unwrap_or_default();
                            backfill(&id, &raw, &mut messages, &tool_index);
                        }
                    }
                }
                continue;
            ...
            for part in parts {
                ...
                match part.get("type").and_then(Value::as_str) {
                    ...
                    // user 行偶带 tool_result(异常路径回灌),同样回填
                    Some("tool_result") => {
                        if let Some(id) = optional_string(data.get("tool_use_id")) {
                            let raw = optional_string(data.get("content")).unwrap_or_default();
                            backfill(&id, &raw, &mut messages, &tool_index);
                    ...
                    // image_url 是 qoder:/// 缓存引用,解码不出图像,跳过
                    _ => {}
                }
            }
            ...
            let meta = row.get("is_meta").and_then(Value::as_bool) == Some(true)
                || is_injected_user_content(&content.text);
            let (text, truncated) = clip(&content.text, MAX_MSG_TEXT);
            messages.push(TranscriptMessage {
                seq: 0,
                role: Role::User,
    ...
    let sidecar = read_task_sidecar(path);
    let cwd = if sidecar.working_dir.is_empty() {
        decode_task_project_dir(path)
    } else {
        sidecar.working_dir
    };
    if created_at == 0 {
    ...
    updated_at = updated_at.max(sidecar.updated_at);
    Ok(QoderParse {
        messages,
        custom_title: sidecar.title,
        ai_title: String::new(),
...
impl AgentAdapter for QoderAdapter {
    ...
    fn list_session_files(&self) -> Result<Vec<SessionFileRef>> {
        ...
        for (ix, root) in self.roots.iter().enumerate() {
            if ix == 0 {
                refs.extend(list_project_tree_refs(root, AgentId::Qoder));
            } else {
                // 任务根:task-*.session.execution.jsonl 落在编码项目目录里,
                // 根直属的老文件也存在;整树浅走,按文件名后缀筛
                for entry in jsonl_entries(root) {
                    let name = entry.file_name().to_string_lossy().to_string();
                    if Self::is_task_file(&name) {
                        if let Some(r) = default_file_ref(AgentId::Qoder, entry.path()) {
                            refs.push(r);
                        }
                    }
                }
    ...
    fn file_ref(&self, path: &Path) -> Option<SessionFileRef> {
        ...
        // 主树:root/<session>.jsonl 或 root/<project-key>/<session>.jsonl；
        // 更深的 state/subagents/memory 均是边车，不得成为顶层会话。
        for root in &self.roots {
            if let Ok(relative) = path.strip_prefix(root) {
                if relative.components().count() > 2 {
                    return None;
                }
                return default_file_ref(self.agent(), path);
            }
        }
        None
    }
    ...
    fn parse_session(&self, r: &SessionFileRef) -> Result<ParsedSession> {
        let parsed = if r.native_id.ends_with(".session.execution") {
            parse_qoder_task_jsonl(Path::new(&r.file_path), false)?
        } else {
            parse_qoder_jsonl(Path::new(&r.file_path), false)?
        };
    ...
    fn parse_transcript(&self, r: &SessionFileRef) -> Result<ParsedTranscript> {
        let parsed = if r.native_id.ends_with(".session.execution") {
            parse_qoder_task_jsonl(Path::new(&r.file_path), true)?
        } else {
            parse_qoder_jsonl(Path::new(&r.file_path), true)?
        };
        Ok(ParsedTranscript {
    ...
    /// 三根彼此独立启停(Session locations 面板逐行开关)
    fn supports_individual_root_removal(&self) -> bool {
        true
    }

    fn excluding_data_roots(&self, excluded: &[PathBuf]) -> Option<Box<dyn AgentAdapter>> {
        let roots: Vec<PathBuf> = self
            ...
            .cloned()
            .collect();
        (!roots.is_empty()).then(|| Box::new(Self { roots }) as Box<dyn AgentAdapter>)
    }
