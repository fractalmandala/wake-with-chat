...
pub fn create_adapters() -> Vec<Box<dyn AgentAdapter>> {
    vec![
        ...
        // Cowork 是 Claude Desktop 的沙盒会话,布局与 Claude Code 同构,
        // 紧跟 Claude 便于排障;独立的 AgentId,不与 Claude Code 去重
        Box::new(cowork::CoworkAdapter::new()),
        ...
        Box::new(kimi::KimiAdapter::desktop()),
        // 桌面端 conversations/*.db 是全文转录,CLI summaries 库只有元数据;
        // 同 id 会话由 scanner 去重,dedup_rank 让转录实例胜出
        Box::new(antigravity::AntigravityAdapter::new()),
        Box::new(antigravity::AntigravityDesktopAdapter::new()),
