...
impl KimiAdapter {
    ...
    /// Kimi Work(kimi-desktop 桌面端):同 wire.jsonl 方言、conv- 前缀会话
    /// 目录,cwd 映射在自家 session_index.jsonl。与 CLI 实例同属 AgentId::Kimi,
    /// 同 id 会话由 scanner 去重裁决。
    pub fn desktop() -> Self {
        let home = super::home_dir().unwrap_or_default().join(
...
/// Kimi Work(kimi-desktop)同一布局但目录前缀是 conv- / ctitle-
/// (会话有标题后改名),一并认可。
fn session_dir_of(wire_path: &Path) -> Option<&Path> {
    let dir = wire_path.ancestors().nth(3)?;
    let name = dir.file_name()?.to_string_lossy();
    (name.starts_with("session_") || name.starts_with("conv-") || name.starts_with("ctitle-"))
        .then_some(dir)
}
