...
impl Store {
    ...
    /// 按 native_id 取某家 agent 库里非空的会话标题。in-app chat 登记簿
    ...
    pub fn titles_by_native_ids(&self, agent: &str, ids: &[String]) -> Result<HashMap<String, String>> {
        ...
        let placeholders = ids
            ...
            .join(",");
        let sql = format!(
            "SELECT native_id, title FROM sessions
             WHERE agent_id = ?1 AND title != '' AND native_id IN ({placeholders})"
        );
        let conn = self.read.lock().unwrap();
        let mut stmt = conn.prepare_cached(&sql)?;
        let mut args: Vec<&dyn rusqlite::ToSql> = vec![&agent];
        for id in ids {
            args.push(id);
        }
